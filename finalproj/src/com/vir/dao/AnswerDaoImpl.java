...
List<Answer> list=new ArrayList<Answer>();  
        ...
        try{  
            ...
            PreparedStatement ps=con.prepareStatement("select qid,empid,ans from  vforum.answer where qid=?");  
            ps.setInt(1, qid);
            ResultSet rs=ps.executeQuery();  
            while(rs.next()){  
            	Answer Answer=new Answer(); 
            	Answer.setQid(rs.getInt("qid"));
            	Answer.setEmpid(rs.getInt("empid"));
            	Answer.setAns_desc(rs.getString("ans"));
            	list.add(Answer);
            }
            con.commit();
            con.close();  
